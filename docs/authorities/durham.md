...
The official council planning page remained available with HTTP 200. The
linked Public Access host did not accept a connection from the qualification
host. `publicaccess.durham.gov.uk` resolved to `217.23.233.121`. Separate
bounded HTTP and HTTPS probes to the portal root timed out after 10 seconds.
A separate collector attempt against `search.do?action=advanced` exhausted its
40-second request budget. Chrome and the in-app browser also timed out before
either page returned an HTTP response.
...
pending. No qualification receipt was emitted. A transient local blocker JSON
was not retained as committed evidence, so this dated walkthrough and the
audit ledger are the durable blocker record.
