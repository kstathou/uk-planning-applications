...
| Haringey | `publicregister.haringey.gov.uk/pr/s/` plus `my.haringey.gov.uk` map services | Salesforce public register plus Arcus and legacy Astun iShare layers | Seven-day discovery, one detail record, one explicit empty comment section, and its six-row file index were verified. Separate live current-layer families provide a credible cross-era discovery boundary, but the reconciled 826-PKID legacy-current layer has dead detail links. The independently reconciled 13,974-PKID decided layer has zero overlap with it, and Salesforce exposes no legacy identifier, so complete collection remains unproved. See `authorities/haringey.md`. |
