...
| Haringey | `publicregister.haringey.gov.uk/pr/s/` plus `my.haringey.gov.uk` map services | Salesforce public register plus Astun iShare map | Seven-day discovery, one detail record, one explicit empty comment section, and its six-row file index were verified. The 1,445-row current-map layer reconciles internally but includes a 2023 decided case, so it is not proof of complete older-open discovery. See `authorities/haringey.md`. |
