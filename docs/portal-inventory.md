...
| Haringey | `publicregister.haringey.gov.uk/pr/s/`, `my.haringey.gov.uk` map services, GLA Planning London Datahub, and national Planning Data | Salesforce public register plus Arcus and legacy Astun iShare layers | Seven-day discovery, one detail record, one explicit empty comment section, and its six-row file index were verified. A machine-first follow-up exhausted both advertised map configurations, the GLA application API, all Haringey-managed national datasets, and 150,770 unique Salesforce sitemap identities. None exposes a PKID join for the 826-record legacy-current cohort; all independently published PKID-to-HGY sets have zero overlap. Complete collection remains unproved. See `authorities/haringey.md`. |
