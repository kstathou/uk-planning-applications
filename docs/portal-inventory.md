...
| Birmingham | Legacy Northgate portal and official `mybrummap` ArcGIS layer 12 | Northgate plus ArcGIS Feature Service | Northgate returned HTTP 503. ArcGIS current-window count and pagination were verified, but freshness continuity, older-open semantics, active appeals, and child sections were not. See `authorities/birmingham.md`. |
...
Arun, Barnet, Camden, Cornwall, Devon, Durham, Haringey, Peak District, and West Suffolk have recorded detail paths. Cheshire East and Leeds have recorded discovery paths but inconclusive detail retrieval. Birmingham has bounded ArcGIS current-window discovery but no provable complete source, older-open set, detail path, or required child sections. Dorset has a verified map-client boundary without bounded application discovery. Blackburn and OPDC are blocked at their entry points. Every authority now has a dated walkthrough, while the unresolved paths remain explicit.
