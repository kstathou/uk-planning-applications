...
from yimby.domain import DiscoveryWindow, EvidenceCapture, EvidenceDigest, FrozenModel
...
class RetainedEvidenceV1(FrozenModel):
    ...
    @classmethod
    def _canonical_media_type(cls, value: object) -> str:
        return canonical_source_media_type(str(value))


class QualificationJournalStageV1(FrozenModel):
    """One request durably prepared before I/O and optionally completed."""

    request: RecordedQueryV1
    state: Literal["prepared", "completed"]
    evidence: RetainedEvidenceV1 | None = None

    @model_validator(mode="after")
    def _state_matches_evidence(self) -> Self:
        if (self.state == "completed") != (self.evidence is not None):
            _raise_invariant("journal-stage-state-mismatch")
        return self


class CheshireEastQualificationJournalV1(FrozenModel):
    """Durable source-stage queue used to resume without replaying completions."""

    schema_version: Literal[1] = 1
    kind: Literal["cheshire-east-qualification-journal"] = (
        "cheshire-east-qualification-journal"
    )
    scope: QualificationScopeV1
    stages: tuple[QualificationJournalStageV1, ...] = ()

    @model_validator(mode="after")
    def _stages_are_a_planned_prefix(self) -> Self:
        keys = tuple(stage.request.key for stage in self.stages)
        if keys != _planned_query_inventory(self.scope)[: len(keys)]:
            _raise_invariant("journal-query-prefix-mismatch")
        _validate_attempted_request_shapes(
            self.scope,
            tuple(stage.request for stage in self.stages),
        )
        prepared = tuple(
            index
        ...
        if prepared not in {(), (len(self.stages) - 1,)}:
            _raise_invariant("journal-stage-order-mismatch")
        return self
...
class _ProbeJournal:
    def __init__(
        self,
        data_dir: Path,
        journal: CheshireEastQualificationJournalV1,
    ) -> None:
        self._data_dir = data_dir
        self._path = data_dir / _JOURNAL_NAME
        self._store = EvidenceStore(data_dir / "evidence")
    ...
    async def capture(
        ...
        recorded = _recorded_query(key, request)
        if self._index < len(self._journal.stages):
            stage = self._journal.stages[self._index]
            if stage.request != recorded:
                _raise_invariant("journal-request-mismatch")
            if stage.state == "completed":
                evidence = stage.evidence
                if evidence is None:
                    _raise_invariant("journal-stage-evidence-missing")
                capture = _read_retained_evidence(self._data_dir, evidence)
                self._index += 1
                return capture
        else:
            stage = QualificationJournalStageV1(
                request=recorded,
                state="prepared",
            )
            self._replace_stages((*self._journal.stages, stage))
        capture = await session.fetch(request)
        evidence = _retain_evidence(self._store, (capture,))[0]
        stages = list(self._journal.stages)
        stages[self._index] = QualificationJournalStageV1(
            request=recorded,
            state="completed",
            evidence=evidence,
        )
        self._replace_stages(tuple(stages))
        self._index += 1
        return capture
    ...
    def _replace_stages(
        ...
        self._journal = CheshireEastQualificationJournalV1(
            scope=self._journal.scope,
            stages=stages,
        )
...
def _open_probe_journal(config: _Config) -> _ProbeJournal:
    path = config.data_dir / _JOURNAL_NAME
    if config.resume and path.is_file():
        journal = CheshireEastQualificationJournalV1.model_validate_json(
            path.read_text(encoding="utf-8")
        )
        if journal.scope != config.scope:
            raise _QualificationConfigError(_INVALID_WINDOW)
        for stage in journal.stages:
            if stage.evidence is not None:
                _read_retained_evidence(config.data_dir, stage.evidence)
    else:
        journal = CheshireEastQualificationJournalV1(scope=config.scope)
        _write_model(path, journal)
    return _ProbeJournal(config.data_dir, journal)
...
async def _probe(
    ...
    async def capture(key: str, request: PortalRequest) -> EvidenceCapture:
        response = await journal.capture(session, key, request)
...
async def _run_probe(
    ...
    try:
        return await _probe(scope, session, journal)
    finally:
...
class _RetainedEvidenceReplay:
    ...
    def parse_stage[Stage](
        ...
        if (
            canonical_source_media_type(self.receipt.evidence[index].media_type)
            != "text/html"
        ):
...
def _read_retained_evidence(
    ...
    return EvidenceCapture(
        ...
        body=body,
        digest=EvidenceDigest(item.digest),
    )
...
def _verify_receipt(
    ...
    if receipt.scope != expected_scope:
        raise _QualificationConfigError(_INVALID_WINDOW)
    bodies = [_read_retained_evidence(data_dir, item).body for item in receipt.evidence]
