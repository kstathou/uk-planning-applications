...
"""Durham-owned fixture and live IDOX Public Access adapter."""
...
class DurhamDocumentV1(FrozenModel):
    """Durham document metadata without attachment bodies."""
...
class DurhamApplicationV1(FrozenModel):
    ...
    proposal_text: str
    case_status: str
    documents: tuple[DurhamDocumentV1, ...]
    ...
    consultee_comment_count: int | None = None


class DurhamAdapter:
    """Own Durham request, parsing, completeness, and normalisation rules."""
    ...
    async def _discover_fixture(
        ...
        next_page = _required_fixture(html, r'data-durham-page="([^"]+)"', "page")
    ...
    async def fetch(
        ...
        """Read fixture detail or Durham live summary and exposed sections."""
        if session.mode == TransportMode.FIXTURE:
    ...
    async def _fetch_fixture(
        ...
        documents = tuple(
            DurhamDocumentV1(title=unescape(title), url=HttpUrl(document_url))
            for title, document_url in re.findall(
                r'data-durham-document="([^"]+)" href="([^"]+)"', html
            )
        )
        payload = DurhamApplicationV1(
            idox_key=_required_fixture(html, r'data-durham-key="([^"]+)"', "IDOX key"),
            application_reference=reference.reference,
            proposal_text=unescape(
                _required_fixture(html, r'data-durham-proposal="([^"]+)"', "proposal")
            ),
            case_status=_required_fixture(
                html, r'data-durham-status="([^"]+)"', "status"
            ),
            electoral_division=_required_fixture(
                html, r'data-durham-division="([^"]+)"', "electoral division"
            ),
            documents=documents,
        )
    ...
    async def _fetch_live(
        ...
        if reference.locator is None:
            raise DurhamRoutingError(reference.reference)
        ...
        if published_reference != reference.reference:
            raise DurhamReferenceMismatchError(reference.reference, published_reference)
        evidence: list[EvidenceCapture] = [summary]
        documents, document_state = await _fetch_documents(
            session, reference.locator, evidence
        )
        consultee_count, comment_state = await _fetch_comment_availability(
            session, reference.locator, evidence
        )
        payload = DurhamApplicationV1(
            ...
            consultee_comment_count=consultee_count,
        )
        return NativeSnapshot(
            reference=reference,
            observed_at=datetime.now(UTC),
...
async def _fetch_documents(
...
) -> tuple[tuple[DurhamDocumentV1, ...], SectionState]:
    try:
        capture = await session.fetch(
    ...
    except DurhamParseError as error:
...
async def _fetch_comment_availability(
    ...
    locator: str,
    evidence: list[EvidenceCapture],
) -> tuple[int | None, SectionState]:
    try:
        capture = await session.fetch(
            _detail_request(locator, "consulteeComments", RequestIntent.COMMENTS)
        )
    except SourceUnavailableError:
        return None, FailedSection(code="source-unavailable")
    evidence.append(capture)
    try:
        count = _section_count(
            BeautifulSoup(capture.body, "html.parser"),
            "consultee comments",
        )
    except DurhamParseError as error:
        return None, FailedSection(code=error.code)
    return count, UnavailableSection(
        reason="Durham public representations are not exposed by the verified record"
    )
...
def _assert_count(section: str, expected: int, actual: int) -> None:
    if expected != actual:
        raise DurhamCountMismatchError(section, expected, actual)
...
class DurhamParseError(ValueError):
    """A required Durham boundary value was absent or inconsistent."""
...
class DurhamCountMismatchError(DurhamParseError):
    """A Durham displayed count disagreed with parsed records."""
...
class DurhamCheckpointError(ValueError):
    """A saved Durham weekly query is absent from the current form."""

    def __init__(self, query: str) -> None:
        """Name the invalid query without exposing session values."""
        super().__init__(f"Durham checkpoint query is unavailable: {query}")


class DurhamRoutingError(ValueError):
    """A Durham live reference lacks its source-local keyVal."""

    def __init__(self, reference: str) -> None:
        """Name the unroutable human reference."""
        super().__init__(f"Durham reference has no keyVal locator: {reference}")


class DurhamReferenceMismatchError(ValueError):
    """The routed Durham record published a different reference."""

    def __init__(self, expected: str, actual: str) -> None:
        """Describe the stable-reference mismatch."""
        super().__init__(
            f"Durham reference mismatch: expected {expected}, got {actual}"
        )
