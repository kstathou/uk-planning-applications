...
"""Cornwall IDOX Public Access adapter."""
...
class CornwallCheckpointV1(FrozenModel):
    """Cornwall IDOX result-page cursor."""
...
class CornwallParseError(ValueError):
    """A required Cornwall field was absent."""
...
class CornwallAdapter:
    """Own Cornwall IDOX fields and document metadata."""
    ...
    async def discover(
        ...
        """Discover Cornwall references from a captured result page."""
        ...
        next_page = _required(html, r'data-cornwall-page="([^"]+)"', "page")
    ...
    async def fetch(
        ...
        """Read Cornwall detail and document metadata only."""
        ...
        payload = CornwallApplicationV1(
            ...
            proposal_text=unescape(
                _required(html, r'data-cornwall-proposal="([^"]+)"', "proposal")
            ),
            case_status=_required(
                html,
                r'data-cornwall-status="([^"]+)"',
                "status",
            ),
            parish_name=_required(
                html,
                r'data-cornwall-parish="([^"]+)"',
                "parish",
            ),
            documents=documents,
        )
    ...
    def normalise(
        ...
        return NormalisedObservation(
            ...
            normaliser_version="cornwall-v1",
        )
