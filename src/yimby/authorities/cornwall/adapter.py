...
"""Cornwall-owned fixture and live IDOX Public Access adapter."""
...
SOURCE = SourceId("cornwall-idox-public-access")
BASE_URL = "https://planning.cornwall.gov.uk/online-applications"
...
class CornwallDocumentV1(FrozenModel):
    """Cornwall document metadata without attachment bodies."""
...
class CornwallApplicationV1(FrozenModel):
    """Cornwall-native IDOX application."""

    idox_key: str
    application_reference: str
    proposal_text: str
    case_status: str
    documents: tuple[CornwallDocumentV1, ...]
...
class CornwallAdapter:
    """Own Cornwall request, parsing, completeness, and normalisation rules."""

    manifest = AuthorityManifest(
        id=AuthorityId("cornwall"),
    ...
    async def discover(
        ...
        """Use fixture discovery or Cornwall's captured weekly IDOX flow."""
    ...
    async def _discover_fixture(
        ...
        references = tuple(
            ...
            for value in re.findall(r'data-cornwall-reference="([^"]+)"', html)
        )
        next_page = _required_fixture(html, r'data-cornwall-page="([^"]+)"', "page")
        yield DiscoveryBatch(
            references=references,
            next_checkpoint=CornwallCheckpointV1(result_page=next_page),
    ...
    async def _discover_live(  # noqa: C901, PLR0912
        ...
        if (
            ...
            raise CornwallCheckpointError(progress.active_query)
        pending = tuple(
        ...
        for query in pending:
            ...
            while True:
                ...
                if row_count > search_page.reported or (
                    ...
                    raise CornwallCountMismatchError(
    ...
    ) -> NativeSnapshot[CornwallApplicationV1]:
        """Read fixture detail or Cornwall live summary and exposed sections."""
        if session.mode == TransportMode.FIXTURE:
    ...
    async def _fetch_fixture(
        ...
        documents = tuple(
            CornwallDocumentV1(title=unescape(title), url=HttpUrl(document_url))
            for title, document_url in re.findall(
                r'data-cornwall-document="([^"]+)" href="([^"]+)"', html
            )
        )
        payload = CornwallApplicationV1(
            idox_key=_required_fixture(
                html, r'data-cornwall-key="([^"]+)"', "IDOX key"
            ),
            application_reference=reference.reference,
            proposal_text=unescape(
                _required_fixture(html, r'data-cornwall-proposal="([^"]+)"', "proposal")
            ),
            case_status=_required_fixture(
                html, r'data-cornwall-status="([^"]+)"', "status"
            ),
            parish_name=_required_fixture(
                html, r'data-cornwall-parish="([^"]+)"', "parish"
            ),
            documents=documents,
        )
    ...
    async def _fetch_live(
        ...
        if reference.locator is None:
            raise CornwallRoutingError(reference.reference)
        ...
        if published_reference != reference.reference:
            raise CornwallReferenceMismatchError(
        ...
        comment_state = await _fetch_comment_availability(
            session, reference.locator, evidence
        )
        payload = CornwallApplicationV1(
    ...
    def normalise(
        self,
        snapshot: NativeSnapshot[CornwallApplicationV1],
    ) -> NormalisedObservation:
        """Map Cornwall-native fields to the common record."""
        ...
        return NormalisedObservation(
            ...
            normaliser_version="cornwall-v2",
...
async def _fetch_documents(
...
) -> tuple[tuple[CornwallDocumentV1, ...], SectionState]:
    try:
        capture = await session.fetch(
    ...
    except CornwallParseError as error:
...
async def _fetch_comment_availability(
    session: PortalSession,
    locator: str,
    evidence: list[EvidenceCapture],
) -> SectionState:
    try:
        capture = await session.fetch(
            _detail_request(locator, "neighbourComments", RequestIntent.COMMENTS)
        )
    except SourceUnavailableError:
        return FailedSection(code="source-unavailable")
    evidence.append(capture)
    try:
        count = _section_count(BeautifulSoup(capture.body, "html.parser"), "comments")
    except CornwallParseError as error:
        return FailedSection(code=error.code)
    if count == 0:
        return EmptySection()
    return UnavailableSection(
        reason=(
            "Cornwall public comment text was not verified beyond the zero-count case"
        )
    )
...
def _assert_count(section: str, expected: int, actual: int) -> None:
    if expected != actual:
        raise CornwallCountMismatchError(section, expected, actual)
...
class CornwallParseError(ValueError):
    """A required Cornwall boundary value was absent or inconsistent."""
    ...
    def __init__(self, field: str) -> None:
        ...
        super().__init__(f"missing Cornwall field {field}")


def _raise_parse(field: str) -> NoReturn:
    raise CornwallParseError(field)


class CornwallCountMismatchError(CornwallParseError):
    """A Cornwall displayed count disagreed with parsed records."""
...
class CornwallOpenEnumerationUnsupportedError(RuntimeError):
    """Older-open Cornwall enumeration lacks a proven bounded partition."""

    def __init__(self) -> None:
        """Prevent weekly discovery from claiming full bootstrap coverage."""
        super().__init__("Cornwall older-open enumeration is not implemented")


class CornwallCheckpointError(ValueError):
    """A saved Cornwall weekly query is absent from the current form."""

    def __init__(self, query: str) -> None:
        """Name the invalid query without exposing session values."""
        super().__init__(f"Cornwall checkpoint query is unavailable: {query}")


class CornwallRoutingError(ValueError):
    """A Cornwall live reference lacks its source-local keyVal."""

    def __init__(self, reference: str) -> None:
        """Name the unroutable human reference."""
        super().__init__(f"Cornwall reference has no keyVal locator: {reference}")


class CornwallReferenceMismatchError(ValueError):
    """The routed Cornwall record published a different reference."""

    def __init__(self, expected: str, actual: str) -> None:
        """Describe the stable-reference mismatch."""
        super().__init__(
            f"Cornwall reference mismatch: expected {expected}, got {actual}"
        )
