# Copyright (c) 2026 Kostas Stathoulopoulos
# ruff: noqa: INP001, T201

"""Shared bounded runner for authority-owned IDOX live smoke scripts."""

from __future__ import annotations

import argparse
import asyncio
import json
from datetime import date, timedelta
from pathlib import Path
from typing import TYPE_CHECKING

from yimby.domain import DiscoveryWindow, FrozenModel, SourceReference, StoredCheckpoint
from yimby.http_transport import HttpxPortalSession

if TYPE_CHECKING:
    from collections.abc import Sequence

    from yimby.adapters import RunnableAuthority


class IdoxSmokeState(FrozenModel):
    """Non-secret checkpoint state for one authority and weekly query."""

    authority: str
    week: date
    checkpoint: StoredCheckpoint | None = None
    first_reference: SourceReference | None = None
    discovered: int = 0


class SmokeConfigurationError(ValueError):
    """The operator did not supply a bounded Monday-based live request."""

    def __init__(self) -> None:
        """Explain the weekly-list boundary."""
        super().__init__("--week must be a Monday")


class SmokeAttachmentPolicyError(RuntimeError):
    """The smoke observed an attachment body request."""

    def __init__(self) -> None:
        """Report the invariant without disclosing response details."""
        super().__init__("attachment body request observed")


class SmokeStateBoundaryError(ValueError):
    """A checkpoint belongs to another authority or weekly query."""

    def __init__(self, expected: str, actual: str) -> None:
        """Prevent state from crossing an authority or date boundary."""
        super().__init__(f"smoke state is for {actual}, not {expected}")


def _parser(authority: str) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=f"Run one explicitly authorised, bounded {authority} live smoke."
    )
    parser.add_argument(
        "--confirm-live",
        action="store_true",
        help=f"explicitly permit live requests to the {authority} public register",
    )
    parser.add_argument(
        "--week",
        required=True,
        metavar="YYYY-MM-DD",
        help="recorded Monday whose seven-day weekly list will be queried",
    )
    parser.add_argument(
        "--state",
        type=Path,
        help="non-secret checkpoint file; defaults below .yimby",
    )
    return parser


def _load_state(path: Path, authority: str, week: date) -> IdoxSmokeState:
    if not path.exists():
        return IdoxSmokeState(authority=authority, week=week)
    state = IdoxSmokeState.model_validate_json(path.read_text())
    expected = f"{authority}/{week.isoformat()}"
    actual = f"{state.authority}/{state.week.isoformat()}"
    if actual != expected:
        raise SmokeStateBoundaryError(expected, actual)
    return state


def _save_state(path: Path, state: IdoxSmokeState) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_suffix(f"{path.suffix}.tmp")
    temporary.write_text(state.model_dump_json())
    temporary.replace(path)


async def _smoke(
    authority: str,
    package: RunnableAuthority,
    week: date,
    state_path: Path,
    expected_detail_errors: tuple[type[Exception], ...],
) -> dict[str, object]:
    if week.weekday() != 0:
        raise SmokeConfigurationError
    session = HttpxPortalSession()
    state = _load_state(state_path, authority, week)
    complete = False
    detail_error: str | None = None
    observation = None
    try:
        window = DiscoveryWindow(
            start=week,
            end=week + timedelta(days=6),
            include_open=False,
        )
        async for batch in package.discover(session, window, state.checkpoint):
            complete = batch.complete
            first_reference = state.first_reference
            if first_reference is None and batch.references:
                first_reference = batch.references[0]
            state = state.model_copy(
                update={
                    "checkpoint": batch.next_checkpoint,
                    "first_reference": first_reference,
                    "discovered": state.discovered + len(batch.references),
                }
            )
            _save_state(state_path, state)
        if state.first_reference is not None:
            try:
                observation = await package.collect(session, state.first_reference)
            except expected_detail_errors as error:
                detail_error = f"{type(error).__name__}: {error}"
        if session.attachment_body_requests != 0:
            raise SmokeAttachmentPolicyError
        return {
            "authority": authority,
            "week": week.isoformat(),
            "complete": complete,
            "discovered": state.discovered,
            "fetched": 0 if observation is None else 1,
            "detail_error": detail_error,
            "reference": (
                None
                if observation is None
                else observation.normalised.reference.reference
            ),
            "requests": len(session.requested_urls),
            "transferred_bytes": session.transferred_bytes,
            "attachment_body_requests": session.attachment_body_requests,
            "state": str(state_path),
        }
    finally:
        await session.aclose()


def run_smoke(
    authority: str,
    package: RunnableAuthority,
    argv: Sequence[str] | None = None,
    *,
    expected_detail_errors: tuple[type[Exception], ...] = (),
) -> int:
    """Validate opt-in arguments and print one structured smoke result."""
    arguments = _parser(authority).parse_args(argv)
    if not arguments.confirm_live:
        result: dict[str, object] = {
            "ok": False,
            "error": f"pass --confirm-live to permit live {authority} requests",
        }
        print(json.dumps(result, sort_keys=True))
        return 2
    state_path: Path | None = arguments.state
    try:
        week = date.fromisoformat(arguments.week)
        if state_path is None:
            state_path = Path(".yimby") / f"smoke-{authority}-{week.isoformat()}.json"
        result = {
            "ok": True,
            **asyncio.run(
                _smoke(
                    authority,
                    package,
                    week,
                    state_path,
                    expected_detail_errors,
                )
            ),
        }
    except (ValueError, RuntimeError) as error:
        result = {
            "ok": False,
            "error_type": type(error).__name__,
            "error": str(error),
        }
        if state_path is not None:
            result["state"] = str(state_path)
    print(json.dumps(result, sort_keys=True))
    return 0 if result["ok"] else 1
