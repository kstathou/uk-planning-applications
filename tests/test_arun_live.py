...
def test_arun_qualification_accepts_a_new_scope_over_cumulative_sqlite_state(
    ...
    source_reference_loader = module._source_evidence_references
    monkeypatch.setattr(
        module,
        "_source_evidence_references",
        lambda _store: {
            record.reference.reference: record.reference for record in retained
        },
    )
...
def test_arun_qualification_fails_cleanly_on_missing_application_captures(
    ...
    with closing(sqlite3.connect(data_dir / "yimby.sqlite3")) as connection:
        connection.execute(
            ...
            SET evidence_digests_json = '[]', evidence_captures_json = '[]'
